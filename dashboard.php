    ...
    <body>
        ...
        <div class="dashboard">
            ...
            <div class="body-section">
				...
				<?php
					$stmt = $pdo->query("SELECT * FROM tbltask ORDER BY taskdate ASC");
				...
				?>
							<table class="task-table">
								<tr class="task-format">
									<td class="task-name"><?php echo htmlspecialchars($row["taskname"]); ?></td>
									<td class="task-desc"><?php echo htmlspecialchars($row["taskdescription"]); ?></td>
									<td class="task-date"><?php echo htmlspecialchars($row["taskdate"]); ?></td>
									<td class="task-button">
										<a class="view-button" href="#popup4">
											<button class="func-button" onclick="viewTask('<?php echo htmlspecialchars($row["taskname"]); ?>')">View</button>
										</a>
											...
											<div class="popup">
												...
												<div class="add-content">
													<form action="#" method="post">
														...
														<div class="field">
															<input type="text" name="taskname" value="<?php echo htmlspecialchars($row["taskname"]); ?>" required>
															<label>Task Name</label>
														</div>

														<div class="field">
															<input type="text" name="taskdate" value="<?php echo htmlspecialchars($row["taskdate"]); ?>" required>
															<label>Date of Completion</label>
														</div>

														<div class="desc-field">
															<input type="text" name="taskdescription" value="<?php echo htmlspecialchars($row["taskdescription"]); ?>" required>
															<label>Task Description</label>
														</div>
                ...
                <div class="all-content">
                    ...
                    <div class="no-alltasks">
                    <?php
					$stmt = $pdo->query("SELECT * FROM tbltask WHERE isimportant = 1 ORDER BY taskdate ASC");
					$importantTasks = $stmt->fetchAll();
					$count = 0;

					if (count($importantTasks) > 0) {
						foreach ($importantTasks as $row) {
                            $taskname = $row['taskname'];
							$count++;
							if ($count % 4 == 0) {
								echo "<br>";
									...
									<div class="task-button">
										...
										<div id="popup5" class="overlay">
											<div class="popup">
												...
												<div class="add-content">
													<form action="#" method="post">
														<input type="hidden" name="taskname" value="<?php echo htmlspecialchars($row["taskname"]); ?>">
														<div class="status" id="statuschange">
															<img src="images/starfilled.png" id="statusImage" onclick="changeImage()">
														</div>
                ...
                <div class="all-content">
                    ...
                    <?php
                    $stmt = $pdo->query("SELECT * FROM tbltaskdeleted WHERE is_active = 1 ORDER BY deleted_date DESC");
...
<?php
	if (isset($_POST['btnUpdate'])) {
		...
		$stmt = $pdo->prepare(
			"UPDATE tbltask SET taskname = :taskname, taskdate = :taskdate, taskdescription = :taskdescription
